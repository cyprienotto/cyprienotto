const filesSrcPrefix = process.env.NODE_ENV === "development" ? "" : "/ottodpc";
export const CONSTANTES = {
  githubPage: "https://ottodpc.github.io/ottodpc/",
  name: "OTTO Dieu-Puissant Cyprien",
  description:
    "5 ans d'expérience en tant que Développeur Full-Stack et Concepteur d'Applications, j'utilise les technologies et outils de la philosophie DevOps depuis 3 ans et je suis déterminé à poursuivre mon évolution dans ce domaine.",
  job: "Cloud Engineer DevOps",
  rythme: "",
  FILES_SRC: {
    favicon: filesSrcPrefix + "/favicon.ico",
    avatar: filesSrcPrefix + "/assets/img/ottodpc.png",
    email: filesSrcPrefix + "/email.svg",
    phone: filesSrcPrefix + "/phone.svg",
    address: filesSrcPrefix + "/address.svg",
  },
  colors: {
    white: "#fff",
    dimgray: {
      100: "#686868",
      200: "#5e5e5e",
      300: "#555",
      400: "#525252",
    },
    black: "#000",
    darkblue: "#002abc",
    gainsboro: "#e3e3e3",
    darkgray: "#9ba1ab",
  },
  fontFamily: {
    montserrat: "Montserrat",
    unna: "Unna",
  },
  borderRadius: {
    "8xs": "5px",
    "10xs-6": "2.6px",
    "10xs-9": "2.9px",
  },
  fontSize: {
    "4xs": "0.56rem",
    "6xs": "0.44rem",
    "5xs": "0.5rem",
    base: "1rem",
    xs: "0.75rem",
    "5xs-5": "0.47rem",
    "6xs-5": "0.41rem",
    "3xs": "0.63rem",
    "2xs": "0.69rem",
  },
  COMPETENCES: [
    {
      theme: "Professionnelles",
      competences: [
        "Communication avec clarté",
        "Empathie",
        "Adaptation au changement",
        "Leadership",
      ],
    },
    {
      theme: "Conteneurisation & orchestration",
      competences: ["Docker", "Kubernetes", "Helm", "Docker Swarm"],
    },
    {
      theme: "Messagerie distribuée",
      competences: ["Apache Kafka", "NATS", "RabbitMQ"],
    },
    {
      theme: "Automatisation & configuration",
      competences: ["Git", "Jenkins", "GitLab CI/CD", "Github CI/CD"],
    },
    {
      theme: "CI/CD",
      competences: ["Git", "Jenkins", "GitLab CI/CD", "Github CI/CD"],
    },
    {
      theme: "Observabilité & surveillance",
      competences: [
        "Prometheus",
        "Grafana",
        "Elasticsearch",
        "Logstash",
        "Kibana",
      ],
    },
    {
      theme: "Réseaux & infra",
      competences: [
        "TCP/IP",
        "Load Balancing",
        "Proxies inverses (Nginx, Traefik)",
      ],
    },
    {
      theme: "Langages de programmation / frameworks / tools",
      competences: [
        "Java (Spring)",
        "PHP (Symfony)",
        "JavaScript (TypeScript)",
        "Angular",
        "React",
        "NodeJS",
      ],
    },
    {
      theme: "Tests unitaires, d'intégration, end-to-end",
      competences: ["Playwright", "Selenium", "Jest"],
    },
    {
      theme: "APIs",
      competences: ["WebSockets", "REST", "GraphQL"],
    },
    {
      theme: "Base de données",
      competences: ["PostgreSQL", "MongoDB", "MySQL", "Redis"],
    },
    {
      theme: "Méthodologies et pratiques",
      competences: [
        "SCRUM / Kanban",
        "SOLID",
        "Continuous Testing",
        "Design Patterns",
        "Programmation fonctionnelle",
        "Programmation réactive",
      ],
    },
  ],
  LANGUES: [
    {
      percent: 100,
      langue: "Français",
    },
    {
      percent: 75,
      langue: "Anglais",
    },
    {
      percent: 100,
      langue: "Sango",
    },
  ],
  HOBBIES: [
    {
      percent: 100,
      name: "Cuisine",
      icon: "👨🏿‍🍳",
    },
    {
      percent: 100,
      name: "Course à pied",
      icon: "🏃🏿‍♂️",
    },
    {
      percent: 100,
      name: "Danse",
      icon: "🕺🏿",
    },
    {
      percent: 100,
      name: "Football",
      icon: "⚽️",
    },
    {
      percent: 100,
      name: "Tech's News",
      icon: "💻",
    },
    {
      percent: 100,
      name: "Jeux vidéo",
      icon: "🎮",
    },
  ],
  EDUCATION: [
    {
      date: "Septembre 2024 - Juillet 2025",
      position: "Master Architecte des systèmes d'informations, Bac +5",
      descriptionText: [],
      competences: [],
      rythme: "",
    },
    {
      link: "https://cfa-insta.fr/",
      date: "Septembre 2023 - Juillet 2024",
      company: "CFA-INSTA",
      position:
        "Master  DevOps - Architecte des systèmes d'informations, Bac +4",
      descriptionText: [],
      competences: [],
      rythme: "",
    },
    {
      link: "https://cfa-insta.fr/",
      date: "Septembre 2022 - Juillet 2023",
      company: "CFA-INSTA",
      position: "Concepteur Développeur d’application, Bac +3",
      descriptionText: [],
      competences: [],
      rythme: "",
    },
    {
      link: "https://www.doranco.fr/",
      date: "Septembre 2020 - Mai 2021",
      company: "Doranco - École Supérieur des technologies",
      position: "Technicien supérieur système et réseaux, Bac+2",
      descriptionText: [],
      competences: [],
      rythme: "",
    },
  ],
  EXPERIENCES: [
    {
      date: "sept. 2023 - aujourd’hui",
      type: "Alternance",
      company: "DATA-TRICKS",
      link: "https://www.data-tricks.com",
      position: "DevOps & Quality Assurance Test automation",
      descriptionText: [
        "Intégration et déploiement continu (CI/CD) pour une livraison rapide et fiable.",
        "Mise en place et gestion de l'automatisation des tests dans les pipelines CI/CD avec Selenium, Jenkins",
        "Collaboration avec les équipes de développement pour assurer des normes de qualité de code.",
        "Surveillance et amélioration de la performance et de la sécurité des applications en production.",
      ],
      competences: [
        "Kubernetes",
        "Ansible",
        "Terraform",
        "Jenkins",
        "Java",
        "AWS",
        "Playwright",
        "Selenium",
        "React.js",
        "Java",
        "TypeScript",
        "JavaScript",
        "PostgreSQL",
        "Docker",
        "CI/CD",
      ],
    },
    {
      date: "mai 2022 - juil. 2023",
      type: "Alternance",
      company: "NV GALLERY",
      link: "https://www.nvgallery.com/",
      position: "Software Engineer Full-Stack Symfony / React",
      descriptionText: [
        "Conception et développement de nouvelles fonctionnalités pour améliorer l'expérience utilisateur et augmenter l'efficacité opérationnelle.",
        "Maintenance applicative pour assurer la continuité du service et répondre aux exigences de performance et de sécurité.",
        "Diagnostic et résolution des bugs pour garantir la fiabilité et la stabilité de l'application",
      ],
      competences: [
        "Docker Compose",
        "Symfony",
        "Kubernetes",
        "React.js",
        "Twig",
        "TypeScript",
        "JavaScript",
        "PHP",
        "MySQL",
        "Docker",
        "CI/CD",
        "A/B Testing",
        "HTML/CSS",
      ],
    },
    {
      date: "Jan. 2022 - mai 2022",
      link: "https://www.savane-mousson.fr/",
      company: "Savane & Mousson",
      type: "Indenpendant",
      position: "Développeur WordPress",
      descriptionText: ["Développement du site"],
      competences: ["WordPress", "HTML/CSS", "PHP", "JavaScript"],
    },
    {
      date: "Octobre 2021 - 2 janvier 2023",
      company: "ALKWELO",
      link: "https://alkwelo.app/",
      type: "Indenpendant",
      position: "Ingénierie DevOps & Développemenet Full-Stack React / Node.js",
      descriptionText: [
        "Développement de nouvelles fonctionnalités sur l'application web & mobile",
        "Maintenance applicative web & mobile",
        "Diagnostiquer et correction des bugs",
      ],
      competences: [
        "React Native",
        "MongoDB",
        "Nginx",
        "Redis",
        "Next.js",
        "Docker",
        "CI/CD",
        "Kubernetes",
        "React.js",
        "TypeScript",
        "Rust",
        "HTML/CSS",
      ],
    },
    {
      date: "Janvier 2021 - Juin 2022",
      link: "https://jeconsommenoir.com/",
      company: "JeConsommeNoir.com",
      type: "Bénévolat",
      position: "Développeur Full-Stack React / PHP - Node.js",
      descriptionText: [
        "Développement du site",
        "Développement du back-office",
      ],
      competences: [
        "React.js",
        "PHP",
        "CI/CD",
        "React.js",
        "TypeScript",
        "Next.js",
        "HTML/CSS",
      ],
    },
    {
      date: "Octobre 2021 - mai 2022",
      link: "https://www.savane-mousson.fr/",
      company: "Savane & Mousson",
      type: "Indenpendant",
      position: "Développeur WordPress",
      descriptionText: ["Développement du site"],
      competences: ["WordPress", "HTML/CSS", "PHP", "JavaScript"],
    },
  ],
};
